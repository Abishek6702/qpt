...
export async function sendContactEmail(prevState, formData) {
  ...
  const dangerousPatterns =
    /<[^>]*>|javascript:|onerror=|onload=|onclick=|<script/i;

  const fields = [name, email, phone, company, message];

  for (const field of fields) {
    if (field && dangerousPatterns.test(field)) {
      return {
        success: false,
        error: "HTML and script tags are not allowed.",
      };
    }
  }
  ...
  if (!user || !pass) {
    ...
    return {
      success: true,
      warning:
        "Gmail environment variables (EMAIL_USER & EMAIL_PASS) are not configured. The submission was logged to the server console.",
    };
  }
  ...
  try {
        ...
        <div style="font-family: sans-serif; padding: 20px; color: #333; max-width: 600px; border: 1px solid #eee; border-radius: 8px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
            ...
            <tr>
              ...
              <td style="padding: 6px 0;">${safePhone || "Not provided"}</td>
            </tr>
            <tr>
              <td style="padding: 6px 0; font-weight: bold;">Company:</td>
              <td style="padding: 6px 0;">${safeCompany || "Not provided"}</td>
            </tr>
          </table>
          <h3 style="margin-top: 25px; color: #4b5563;">Message:</h3>
          <p style="white-space: pre-wrap; background-color: #f3f4f6; padding: 15px; border-radius: 6px; border-left: 4px solid #2563EB; font-size: 14px; line-height: 1.6;">${safeMessage}</p>
        </div>
      `,
    });
    ...
    return {
      success: false,
      error:
        "Failed to send email. Please try again later or contact us directly at " +
        adminEmail,
    };
  }
}
